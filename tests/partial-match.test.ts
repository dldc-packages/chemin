...
import { chemin, pString, partialMatch } from '../src/mod';
...
test('Example', () => {
  ...
  const match1 = partialMatch(workspaceHome, workspaceHome.match('/workspace/123/home'), workspace);
  // match1 is typed as { tenant: string } | null
  expect(match1).toMatchObject({ tenant: '123' });

  const match2 = partialMatch(workspaceSettings, workspaceSettings.match('/workspace/123/settings'), workspace);
...
test('partial match', () => {
  ...
  expect(partialMatch(route1, route1Match, userPart)).toMatchObject({ id: '456' });
  expect(partialMatch(route2, route2Match, userPart)).toMatchObject({ id: '123' });
