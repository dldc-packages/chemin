...
function optional<N extends string, T extends any>(sub: CheminParam<N, T>): CheminParam<N, OptionalValue<T>> {
  return {
    name: sub.name,
    match: (...all) => {
...
function optionalConst<N extends string>(name: N, constant: string = name): CheminParam<N, boolean> {
  return {
    name,
    match: (...all) => {
      if (all[0] === constant) {
